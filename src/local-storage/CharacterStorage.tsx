    ...
    try {
      ...
      const cleanedCharacters = data.characters.map(char => ({
        ...
        statusEffects: char.statusEffects.map(statusEffect => {
          ...
          const se = statusEffect as Record<string, any>;

          // Old format: { effect: { name, description, effects, x?, y? }, x?, y? }
          if (se.effect && typeof se.effect === 'object' && se.effect.name) {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const { effects, ...effectCore } = se.effect as Record<string, unknown>;
            return {
              ...effectCore,
              ...(se.x !== undefined ? { x: se.x } : {}),
              ...(se.y !== undefined ? { y: se.y } : {})
            } as ActiveStatusEffect;
          }

          // Current flat format: { name, description, effects?: [garbage], x?, y? }
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          const { effects, ...cleanEffect } = se;
          return cleanEffect as ActiveStatusEffect;
        })
      }));
